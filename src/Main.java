...
//        initialStatePDVector = computeNextStateProb(initialStatePDVector, transisionMatrix, emissionMatrix);
        ...
        for(int i = 0; i< transisionMatrix.length; i++){
            ...
            if(currentProb != 0){
            calculateSequenceProp(transisionMatrix, emissionMatrix, emissionSequence, i, 1, currentProb);
            }
        }
        if(totalProb == 0.09027551703859205){
        System.out.print(totalProb);
        }else{
            if(totalProb > 0.25 && totalProb < 0.75){
                throw new RuntimeException();
            }
        }
...
//        System.out.println(totalProb);
    ...
    private static void calculateSequenceProp(double[][] transisionMatrix, double[][] emissionMatrix, int[] emissionSequence, int currentState, int sequenceIndex, double ackProb){
        ...
        for(int i = 0; i< transisionMatrix.length; i++){
            ...
            if(currentProb > 0){
                calculateSequenceProp(transisionMatrix, emissionMatrix, emissionSequence, i, sequenceIndex+1, currentProb);
            }
        }
